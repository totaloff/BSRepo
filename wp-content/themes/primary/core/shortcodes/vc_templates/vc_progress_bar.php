...
if ($view == 'outside') {
	$phoenixteamClass = ' phoenixteam-team-progerssbar-outside';
} else {
	$phoenixteamClass = ' phoenixteam-team-progerssbar';
}
...
$output = '<div class="' . $css_class . $phoenixteamClass .'">';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_single_bar' . esc_attr($bgcolor) . '">';
	$output .= '<small class="vc_label">' . wp_kses_post($line['label'] . $unit) . '</small>';
	...
	$output .= '<span class="vc_bar' . esc_attr($bar_options) . '" data-percentage-value="' . ( esc_attr($percentage_value) ) . '" data-value="' . esc_attr($line['value']) . '"' . esc_attr($line['bgcolor']) . '></span>';
	$output .= '</div>';
}
