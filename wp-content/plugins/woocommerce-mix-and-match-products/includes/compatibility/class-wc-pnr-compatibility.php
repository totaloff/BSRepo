	...
	public static function points_rewards_mnm_message( $message, $points_n_rewards ) {
		...
		if ( $product->is_type( 'mix-and-match' ) && self::$single_product_message_filter_active ) {
			...
			if ( false === self::has_fixed_points( $product ) && $product->is_priced_per_product() ) {
				...
				add_filter( WC_MNM_Core_Compatibility::is_wc_version_gte( '3.0.0' ) ? 'woocommerce_product_get_price' : 'woocommerce_get_price', array( __CLASS__, 'replace_price' ), 9999, 2 );
				...
				remove_filter( WC_MNM_Core_Compatibility::is_wc_version_gte( '3.0.0' ) ? 'woocommerce_product_get_price' : 'woocommerce_get_price', array( __CLASS__, 'replace_price' ), 9999, 2 );
