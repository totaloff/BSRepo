...
class WC_MNM_Install {
	...
	public static function install() {
		...
		if ( false === $mnm_term_exists = get_term_by( 'slug', 'mix-and-match', 'product_type' ) ) {
			wp_insert_term( 'mix-and-match', 'product_type' );
		}
	...
	private static function update() {
		...
		foreach ( self::$db_updates as $version => $update_callbacks ) {
			if ( version_compare( self::$current_db_version, $version, '<' ) ) {
				WC_MNM_Core_Compatibility::log( sprintf( 'Updating to version %s.', $version ), 'info', 'wc_mnm_db_updates' );
				foreach ( $update_callbacks as $update_callback ) {
					WC_MNM_Core_Compatibility::log( sprintf( '- Queuing %s callback.', $update_callback ), 'info', 'wc_mnm_db_updates' );
	...
	public static function force_update() {
		/**
		 * 'wp_wc_mnm_updater_cron' action.
		 */
		do_action( 'wp_wc_mnm_updater_cron' );
	...
	public static function update_complete() {
		...
		WC_MNM_Core_Compatibility::log( 'Data update complete.', 'info', 'wc_mnm_db_updates' );
		self::update_db_version();
		delete_option( 'wc_mnm_update_init' );
	...
	private static function update_db_version( $version = null ) {
		...
		WC_MNM_Core_Compatibility::log( sprintf( 'Database version is %s.', get_option( 'wc_mix_and_match_db_version', 'unknown' ) ), 'info', 'wc_mnm_db_updates' );
	}
